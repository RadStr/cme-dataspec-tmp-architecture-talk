    ...
    useEffect(() => {
        if(Object.keys(highlightLevels).length > 0) {
            ...
            Object.entries(highlightLevels).forEach(([nodeId, level]) => {
                ...
                if(level === 0) {
                    highlightedNodes.push({...node, style: {...node.style, opacity: 1, backgroundColor: "rgba(238, 58, 115, 1)"}});
                }
                else {
                    highlightedNodes.push({...node, style: {...node.style, backgroundColor: "rgba(138, 158, 15, 1)"}});
                }
            });
            setNodes(prev => {
                return prev.map(n => {
            ...
            // const node = reactFlowInstance.getNode(Object.keys(highlightLevels)[0] ?? "");
            // if(node === undefined) {
            //     return;
            // }
            // const connectedEdges = getConnectedEdges([node], edges);
            // const highlightedNodes: Node[] = [];
            // // TODO: Just a test of the edges
            // // const highlightedEdges: Edge[] = [];
            // connectedEdges.forEach(edge => {
            //     const otherNode = reactFlowInstance.getNode(edge.source === node.id ? edge.target : edge.source);
            ...
            //         return;
            //     }
            //     otherNode!.style = {...otherNode?.style, opacity: 0.0};
            //     highlightedNodes.push({...otherNode});
            //     // highlightedEdges.push({...edge, style: {...edge.style, opacity: 0.0}});
            // });
            // highlightedNodes.push({...node, style: {...node.style, opacity: 0.5, backgroundColor: "rgba(238, 58, 115, 1)"}});
            // setNodes(prev => {
            //     return prev.map(n => {
            //         const nn = highlightedNodes.find((nnn) => nnn.id === n.id);
            //         if(nn !== undefined) {
        ...
        return () => {
            ...
            //         return;
            //     }
            //     otherNode!.style = {...otherNode?.style, opacity: 1};
            //     highlightedNodes.push({...otherNode});
            // });

            // highlightedNodes.push({...node, style: {...node.style, opacity: 1, backgroundColor: "rgba(0, 0, 0, 0)"}});
            // setNodes(prev => {
            //     return prev.map(n => {
            //         const nn = highlightedNodes.find((nnn) => nnn.id === n.id);
            //         if(nn !== undefined) {
            ...
            <div className="h-[80vh] w-full md:h-full">
                <ReactFlow
                    ...
                    onNodeMouseLeave={(event, node) => {
                        ...
                        //     const highlightedNodes: Node[] = [];
                        //     connectedEdges.forEach(edge => {
                        //         const otherNode = reactFlowInstance.getNode(edge.source === hoveredNode.id ? edge.target : edge.source);
                        //         if(otherNode !== undefined) {
                        //             otherNode.style = {};
                        //         }
                        //         else {
